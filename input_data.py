...
def check_conflict_fd(fds,table_name,dic_para):
    conflict_fd = []
    for fd in fds:
       isvalid = check_format(fd)
       if(isvalid == True):
            check_wrong_fd(fd)
...
def check_table_key(table_name,key):
    if(key.upper() in key_dic.get(table_name).get('keys')):
        print("Key found")
    else:
        print('Key must be from ',key_dic.get(table_name).get('keys'))
        ask_for_keys()
...
def check1Nf(keys,lhs,rhs):
    ...
    for k in keys:
        for l in range(len(lhs)):
            ...
            print("lhsAttrs::",lhsAttrs)
            print("keyAttrs::",keyAttrs)
            isNotKey = check_equal(lhsAttrs,keyAttrs)
            list(filter(lambda x: x not in keyAttrs,lhsAttrs))
            print("isNotKey",isNotKey)
            print("len lhs attr::",len(lhsAttrs))
            print("isRhsPartialKey",isRhsPartialKey(rhs[l],keys))
